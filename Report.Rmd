---
title: "Report"
author: Samantha Xinwei Han (xh2601), Xinhui Lin (xl3054), Ruohan Lyu (rl3610), Xinyin Miao (xm2356), Fenglin Xie (fx2212)
date: "2025-12-06"
...
Because reports depend on when humans are awake, outdoors, and likely to encounter animals, temporal patterns primarily represent reporting frequency, not animals themselves. Given these insights, we shifted our analytical focus to a question the dataset can meaningfully answer: How do human response behaviors (ranger response time) vary with animal condition, number of animals, time of day, or borough? This reframing allowed us to analyze variables that truly reflect actionable patterns in the dataset.
...
<div style="text-align:center;">
  <img src="images/map_boro.jpg" 
       style="width:90%; margin:10px auto 0 auto; display:block;">
</div>
...
We would like to explore how response duration, which we define as the duration between the initial report and the time that Rangers arrived at the site (`date_and_time_of_ranger_response` - `date_and_time_of_initial_call`), is associated with different incident characteristics.

<br>

#### Average Response Duration by Time of Initial report
...
ranger_response_hr %>% 
  ...
  labs(
    title = "Average Response Duration by Time of Initial Report",
...
- Cases reported during regular daytime hours (approximately 8:00 - 16:00) have the shortest response duration, typically around 2-4 hours from the time of initial report to the arrival of a Ranger. This pattern suggests that Ranger availability and operational capacity are highest during standard working hours.
...
- Queens, the Bronx, and Staten Island have slightly left-shifted peaks on the log scale, indicating somewhat faster responses on average. Operationally, these boroughs may experience less congestion and fewer competing reports, contributing to slightly shorter response duration.
...
- Unhealthy and Injured animals show substantial variability in `action duration`, with the highest median durations. The wide distribution reflects the need for diverse handling, such as rehabilitator, ACC and monitored.
...
We were interested in examining the relationship between two categorical variables: species status and health condition. The expectation was that exotic animals might face living environments that differ from those of native species, potentially influencing their condition patterns. Therefore, we conducted a chi-square test of independence to evaluate whether species status is associated with differences in reported health conditions.
...
- The interaction is significant (p = 0.000165), meaning the impact of `action type` depends on the `animal condition`.
...
These insights allow for proactive management to align ranger schedules and resources with peak activity periods. For example, increasing patrols on summer afternoons would help address the higher case frequency across species. Public education efforts can also be timed to raise awareness of seasonal animal behaviors, reducing potential conflicts.  Ultimately, this data-driven approach enhances operational efficiency and wildlife protection in urban ecosystems.

### Health Condition Pattern
...
Patterns in action duration highlight clear differences in operational demands across incident types. Rehabilitation-related actions take the longest, while advisory, monitoring, and relocation tasks are resolved quickly. In final actions, Unfounded means either that the animals were not found or that no further action will be taken. Duration also varies by animal condition: unhealthy and injured animals often require ACC transfer or rehabilitation, whereas DOA cases are consistently brief. Healthy animals show short median durations but unusually wide spread, indicating that simple calls sometimes escalate into complex responses. The ANOVA results reinforce these patterns—final ranger action is the strongest predictor of duration, and after log transformation, animal condition also becomes significant, with a meaningful interaction showing that the effect of action type depends on the animal’s condition.
...
Several data limitations may influence how the results should be interpreted. Some animal condition or animal class categories contain relatively few observations, which can make group comparisons and regression estimates less stable. There are also inconsistencies in the recorded timestamps. For example, there are incidents where the `date_and_time_of_ranger_response` occurs before the `date_and_time_of_initial_call`, which is not feasible and suggests recording errors. These issues introduce additional uncertainty and may contribute to unexplained variability in the findings.
...
Overall, the analysis demonstrates how wildlife rescue activity in urban parks is influenced by a combination of spatial patterns, temporal trends, and certain animal-condition characteristics. Reporting frequency varies distinctly by borough and dominant animal class; case volume peaks during daylight hours and seasonal windows of high animal activity; reported health conditions reflect both species visibility and public detection patterns; response times are largely driven by initial report time and call source, instead of animal characteristics; and final ranger action varies significantly by animal conditions. 

Although the findings generally align with ecological and behavioral expectations, they also challenge some of our assumptions about the role of animal-related traits in shaping rescue behavior. Spatially, case distributions reflect borough characteristics, such as higher aquatic incidents in coastal Staten Island. Temporally, cases peak in summer and during daytime hours, matching wildlife and public activity patterns. Health reports correspond to species visibility - birds are more often reported with visible injuries, while concealed small mammals are found in poorer condition. Several findings diverged from our initial expectations. Response times were influenced more strongly by call source and time of day than by any animal traits. Similarly, procedural factors were the dominant influence on the amount of time rangers spent on site, whereas animal condition had only a small effect and the number of animals involved did not significantly change the time required. 

We initially expected to identify clearer patterns and predictors of animal injury, but our analyses ultimately revealed more about human reporting behaviors and ranger responses. This reflects the nature of the dataset, which captures only incidents reported by people rather than all events affecting wildlife, showing the central role of human awareness in shaping the data.

To summarize, these patterns provide a data-informed foundation for understanding human–wildlife interactions in New York City and illustrate how various factors contribute to incident occurrence as well as ranger response process.
