...
custom_colors = c("#0868ac", "#43a2ca", "#7bccc4", "#bae4bc", "#f0f9e8")

# --- health_by_class dataset ---
health_by_class =
  park_ranger_new |>
...
p2 <- plot_ly(
    ...
    text  = ~hover_text,
    hoverinfo = "text",
    showlegend = FALSE   # <- only one legend kept!
) |>
  layout(
    title  = "By Borough",
    barmode = "stack",
...
fig <- subplot(p1, p2, nrows = 1, shareY = FALSE) |>
  layout(
    title = "Health Conditions Overview",
    showlegend = TRUE,
    legend = list(
      orientation = "h",
      x = 0.5,
    ...
    height = 550,
    margin = list(b = 150)  # ensure text does not cut off
  )
...
1. Actions Involving Rehabilitation Have the Longest Action Durations

Final actions such as “Released back into Park after Rehabilitation” and “Rehabilitator” show the highest median action durations, as well as the widest spread of times.
...
Because they involve limited physical intervention, these actions tend to be resolved quickly, reflected in the tight, compact violins and small IQRs.

3. “Unfounded” and “ACC” Actions Show High Variability
...
The violin-and-boxplot visualization shows how action time varies across different animal conditions. 

1. Unhealthy and Injured Animals Tend to Receive Longer Actions

The Unhealthy and Injured categories show slightly higher median action durations and wider distributions.
...
2. Healthy Animals Show Shorter and More Consistent Action Times

The Healthy category has a shorter median action duration and a narrower distribution.
...
The mean duration of unfounded actions is approximately 1.0 hour, confirming their shorter nature.
...
Final ranger action is highly significant (p < 2e-16), indicating strong differences in action duration across action types.
...
Because action duration is highly right-skewed with many long-tail outliers, ANOVA assumptions were violated when using raw duration values. This motivated the use of a log-transformation to stabilize variance and improve normality of residuals.
...
The log-transformation compresses extreme high-duration actions and spreads out low values, resulting in more homogeneous variance across fitted values , more normal residual distribution, clearer signal for predictors previously overshadowed by extreme right-skewed values.
